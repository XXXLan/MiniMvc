...
namespace MiniMVC
{
    public class DefaultModelBinder:IModelBinder
    {
            ...
            string modelName, Type modelType, out object value)
        {
            ...
            key = controllerContext.ReuqestContext.RouteData.Values
                .Where(item => string.Compare(item.Key, modelName, true) == 0)
                .Select(item => item.Key).FirstOrDefault();
            if (null != key)
            {
                value = Convert.ChangeType(controllerContext.ReuqestContext.RouteData.Values[key], modelType);
                return true;
            }

            key = controllerContext.ReuqestContext.RouteData.DataTokens
                .Where(item => string.Compare(item.Key, modelName, true) == 0)
                .Select(item => item.Key).FirstOrDefault();
            if (null != key)
            {
                value = Convert.ChangeType(controllerContext.ReuqestContext.RouteData.DataTokens[key], modelType);
                return true;
            }
            value = null;
            return false;
